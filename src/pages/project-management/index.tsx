      ...
      extra={
        <div>
          <Button
            type="link"
            onClick={() =>
              navigate(PATH_ENUM.PROJECT_DETAIL.replace(':id', project.projectNumber.toString()))
            }
          >
            查看
          </Button>
    ...
    >
      <div className={styles.cardContent}>
        ...
        {project.projectName ? (
          <p>
            <strong>项目名称：</strong>
            <Link to={PATH_ENUM.PROJECT_DETAIL.replace(':id', project.projectNumber.toString())}>
              {project.projectName}
            </Link>
          </p>
        ) : null}
        ...
        {project.startTime && project.endTime ? (
          <p>
            <strong>计划时间：</strong>
            {project.startTime?.split('T')[0]} ~ {project.endTime?.split('T')[0]}
          </p>
        ) : null}
        {project.taxFee ? (
          <p>
        ...
        {project.projectAudit ? (
          <p>
            <strong>项目负责人审核费：</strong>
            {project.projectType}
          </p>
        ) : null}
        {project.techAudit ? (
          <p>
            <strong>技术负责人审核费：</strong>
            {project.techAudit}
          </p>
        ) : null}
        {project.firstAudit ? (
        ...
        {project.technicalVenture ? (
          <p>
            <strong>技术风险：</strong>
            {project.technicalVenture}
          </p>
        ) : null}
        {project.industryVenture ? (
          <p>
            <strong>行业风险：</strong>
            {project.industryVenture}
          </p>
        ...
        {project.projectScale ? (
          <p>
            <strong>项目投资规模：</strong>
            {project.projectScale}
          </p>
        ) : null}
        {project.expert ? (
          <p>
            <strong>是否外聘专家：</strong>
            {project.expert}
          </p>
        ) : null}
        {project.environment ? (
        ...
        {project.auditQuantity ? (
          <p>
            <strong>送审数量：</strong>
            {project.auditQuantity}
          </p>
        ) : null}
        {project.backupQuantity ? (
          <p>
            <strong>备案数量：</strong>
            {project.backupQuantity}
          </p>
