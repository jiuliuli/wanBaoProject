...
import { PROJECT_SOURCE_TYPE_TEXT, SCALE_TYPE_TEXT, VENTURE_FACTOR_TYPE_TEXT, VENTURE_TYPE_TEXT } from "@/constants/project.constants";
    ...
    const [submitState, doFetch] = useAsyncFn(
        async values => {
            ...
            try {
                ...
                message.success(type === 'edit' ? '更新项目成功' : '新建项目成功');
                navigate(PATH_ENUM.PROJECT_MANAGEMENT);
            } catch (error) {
                message.error(type === 'edit' ? '更新项目失败' : '新建项目失败');
            }
        },
        [type],
    );
