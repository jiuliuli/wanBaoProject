...
import BasicInfo from '@/pages/ongoing-projects/childrenPage/BasicInfo';
...
import OngoingProjectService from '@/services/ongoing-project.service';
...
export default function OngoingProjectsDetail() {
  const { id } = useParams();
  ...
  const materialState = useAsync(async () => {
    if (id) {
      return await OngoingProjectService.fetchMaterialById(id);
    }
  }, [id]);
  ...
  return (
    ...
    !materialState.loading && (
      <PageHeader
        ...
        onBack={() => navigate(PATH_ENUM.ONGOING_PROJECTS_LIST)}
